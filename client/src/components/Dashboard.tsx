      ...
      <div className="dashboard-container">
        <div className="dashboard-header">
          ...
          <p className="dashboard-subtitle">
            Welcome to your GitHub Repo Explorer dashboard
          </p>
        </div>
        ...
        {repos
          .slice((currentPage - 1) * repoPerPage, currentPage * repoPerPage)
          .map((repo: any) => (
            <div key={repo.id}>
              <h3>{repo.name}</h3>
              ...
              <button onClick={() => handleAddToFavorites(repo)}>
                Add to Favorites
              </button>
            </div>
          ))}
        ...
        {repos.length > 0 && (
          <div className="pagination-controls">
            ...
            <button
              onClick={() =>
                setCurrentPage((prev) =>
                  prev < Math.ceil(repos.length / repoPerPage)
                    ? prev + 1
                    : prev,
                )
              }
          ...
          {favorites.length === 0 ? (
            ...
            <>
              <ul className="repo-list">
                {favorites
                  .slice(
                    (favoriteCurrentPage - 1) * favoritesPerPage,
                    favoriteCurrentPage * favoritesPerPage,
                  )
                  .map((repo: any) => (
              ...
              {favorites.length > 5 && (
                <div className="pagination-controls">
                  <button
                    onClick={() =>
                      setFavoriteCurrentPage((prev) => Math.max(prev - 1, 1))
                    }
                  ...
                  <button
                    onClick={() =>
                      setFavoriteCurrentPage((prev) =>
                        prev < Math.ceil(favorites.length / favoritesPerPage)
                          ? prev + 1
                          : prev,
                      )
                    }
